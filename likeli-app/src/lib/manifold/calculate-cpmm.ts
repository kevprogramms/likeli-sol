// lib/manifold/calculate-cpmm.ts
// EXACT MANIFOLD COPY - Core CPMM calculations
...
import { Answer, LimitBet, Fill, Maker } from './types'
...
// Multi-choice sum-to-one sale wrapper (FROM MANIFOLD)
export function calculateCpmmMultiSumsToOneSale(
    ...
    // Import from calculate-cpmm-arbitrage.ts
    const {
        calculateCpmmMultiArbitrageSellYes,
        calculateCpmmMultiArbitrageSellNo
    } = require('./calculate-cpmm-arbitrage')
    ...
    const buyAmount = sumBy(newBetResult.takers, (taker: any) => taker.amount)
    // Transform buys of opposite outcome into sells.
    const saleTakers = newBetResult.takers.map((taker: any) => ({
        ...taker,
        shares: -taker.shares,
        amount: -(taker.shares - taker.amount),
        isSale: true,
    ...
    const saleValue = -sumBy(saleTakers, (taker: any) => taker.amount)
...
// ============================================
// MULTI-CHOICE LIQUIDITY FUNCTIONS (FROM MANIFOLD)
// ============================================
...
export function addCpmmMultiLiquidityToAnswersIndependently(
    ...
    const result: { [answerId: string]: { YES: number; NO: number } } = {}
    for (const [answerId, pool] of Object.entries(pools)) {
        result[answerId] = addCpmmLiquidityFixedP(pool, amountPerAnswer).newPool
    }
    return result
}

// Note: EPSILON is already imported from ./util/math
...
// ============================================
// LP WEIGHT TRACKING (FROM MANIFOLD)
// ============================================
...
export function getCpmmLiquidityPoolWeights(liquidities: LiquidityProvision[]) {
    ...
    const liquiditiesByUser: { [userId: string]: LiquidityProvision[] } = {}
    for (const liq of liquidities) {
        if (!liquiditiesByUser[liq.userId]) liquiditiesByUser[liq.userId] = []
        liquiditiesByUser[liq.userId].push(liq)
    }

    const userAmounts: { [userId: string]: number } = {}
    for (const [userId, userLiqs] of Object.entries(liquiditiesByUser)) {
        const total = userLiqs.reduce((sum, l) => sum + l.amount, 0)
        userAmounts[userId] = Math.max(0, total)
    }

    const totalAmount = Object.values(userAmounts).reduce((a, b) => a + b, 0)

    if (totalAmount === 0) {
        const firstUser = liquidities.sort((a, b) => a.createdTime - b.createdTime)[0].userId
        return { [firstUser]: 1 }
    }

    const weights: { [userId: string]: number } = {}
    for (const [userId, amount] of Object.entries(userAmounts)) {
        if (amount > 0) weights[userId] = amount / totalAmount
    }
    return weights
}
...
export function getUserLiquidityShares(
    ...
    const result: { [outcome: string]: number } = {}
    for (const [outcome, shares] of Object.entries(pool)) {
        result[outcome] = userWeight * shares
    }
    return result
}
